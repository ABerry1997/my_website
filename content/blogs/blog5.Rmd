...
The inverted yield curve has a reputation as being a strong predictor of recessions and its presence is often met with a level of panic. 

The yield curve's presence as a harbinger of recession can be doubted: an idea we hope to explore. 

Using US data from the FRED database, we will explore this doubt surrounding yield curve inversion.
...
ggplot(new_yield_curve, aes(x = maturity, 
                        y = price,
                        color= Year)) +
...
Using the code below , be create a dataframe with all US recessions since 1946
...
geom_ribbon(aes(ymin = pmin(difference,0), 
  ...
  labs(title="Yield Curve Inversion: 10-year minus U.S. Treasury rates", 
       subtitle = "Difference in % points, monthly averages.\nShaded areas correspond to recessions",
       x="",
       y="Difference (10 year-3month) yield in %")

```
As can be seen, the yield curve inversion in almost all cases predicts a recession. However, it must be maintained that it cannot be used as a 100% accuracte indicator and instead should be used as a strong indicator to support other research. 
